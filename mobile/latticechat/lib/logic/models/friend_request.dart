...
class _FriendRequestsPageState extends State<FriendRequestsPage> {
  ...
  String _requestUserId(dynamic request) {
    try {
    ...
    try {
      final dynamic id = request.userId;
      if (id != null) return id.toString();
    } catch (_) {}

    if (request is Map<String, dynamic>) {
      return (request['id'] ?? request['_id'] ?? request['userId'] ?? '')
          .toString();
    }
  ...
  String _requestUsername(dynamic request) {
    try {
      final dynamic username = request.username;
    ...
    try {
      final dynamic name = request.name;
    ...
    if (request is Map<String, dynamic>) {
      return (request['username'] ?? request['name'] ?? 'Unknown User')
          .toString();
    }
