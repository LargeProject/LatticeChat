...
export function WebsocketProvider({ children, wsUrl, onError }: WebsocketProviderProps) {
  ...
  const emitWithAck = useCallback(<R = any>(event: string, data?: any, timeoutMs: number = 5000): Promise<R> => {
      ...
      try {
        ...
        sock.emit(event, data, (err: any, res: R) => {
          ...
          if (err !== undefined && err !== null) {
            ...
            if (typeof err === 'object' && (('success' in err) || ('error' in err))) {
              actualRes = err;
          ...
          if (actualErr) {
            reject(actualErr instanceof Error ? actualErr : new Error(JSON.stringify(actualErr)));
          } else {
            resolve(actualRes);
          }
        });
  ...
  const performHandshake = useCallback(async (socket: Socket) => {
    ...
    try {
      const response = await emitWithAck<{ jwt?: string }, { success?: boolean; userId?: string; error?: string }>('initHandshake', {
        jwt,
        id,
      }, 5000);
      ...
      setError(errorMessage);
      setConnectionState('error');
      console.error('[Websocket] handshake failed', err, 'message:', errorMessage);
      socket.disconnect();
    }
  }, [emitWithAck]);
