...
function CryptoLattice({
  ...
  return (
    <group ref={group} position={position}>
      ...
      <lineSegments>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[lines, 3]} usage={THREE.StaticDrawUsage} />
        </bufferGeometry>

        <lineBasicMaterial color="#5aa9ff" transparent opacity={0.6} depthWrite={false} depthTest={false} />
      </lineSegments>
    </group>
  );
}
...
export default function LatticeAnimation({
  ...
  return (
    <Canvas camera={{ position: [0, 0, zoom], fov: 100 }}>
      <OrbitControls enableRotate enableZoom enablePan autoRotate={autoRotateCamera} />

      <CryptoLattice animate={animateLattice} position={position} />
    </Canvas>
  );
}
